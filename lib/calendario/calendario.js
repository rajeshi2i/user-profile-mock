;( function( $, window, undefined ) {	
	...
	$.Calendario.prototype = {
      ...
      var self = this;
			this.$el.on( 'click.calendario', 'div.fc-row > div', function($event) {
				var $cell = $( this ),
					idx = $cell.index(),
				...
				if( dateProp.day ) {self.options.onDayClick( $event, $cell, $content, dateProp );}
			} );
		},
			...
			for ( var i = 0; i < 7; i++ ) {
				for ( var j = 0; j <= 6; j++ ) {
					...
					if ( day <= monthLength && ( i > 0 || j >= p ) ) {
						...
						if( content !== '' ) {inner += '<div class="hidden-c">' + content.key + '</div>';}
						++day;
					}
					var cellClasses = today ? 'fc-today fc-day' : 'fc-day ';
					if( content !== '' ) {cellClasses += 'fc-content '+content.status+' ';}
